...
namespace Sinbad
{
    ...
    public static class CsvUtil
    {
        ...
        public static List<T> LoadObjects<T>(string filename, string prefixCheck = null, bool strict = true) where T : new()
        {
            using (var stream = File.OpenRead(filename))
            {
                using (var rdr = new StreamReader(stream))
                {
                    return LoadObjects<T>(rdr, prefixCheck, strict);
                }
            }
        }
        ...
        // @param strict If true, log errors if a line doesn't have enough
        //   fields as per the header. If false, ignores and just fills what it can
        public static List<T> LoadObjects<T>(TextReader rdr, string prefixCheck = null, bool strict = true) where T : new()
        {
            var ret = new List<T>();
            ...
            {
                if (prefixCheck != null && !line.StartsWith(prefixCheck))
                {
                    continue;
                }
                var obj = new T();
